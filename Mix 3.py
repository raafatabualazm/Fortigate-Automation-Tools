...
for i in range(len(lists)):
    for j in range(i+1, len(lists)):
        for k in range(j+1, len(lists)):
            new_groups.append(open("{0}&{1}&{2}.txt".format(lists[i].name[:-4],lists[j].name[:-4], lists[k].name[:-4]), 'a'))
...
for i in range(len(lists)):
    for j in range(i+1, len(lists)):
        for k in range(j+1, len(lists)):
            common = users[i] & users[j] & users[k]
            ...
            for u in common:
                ...
                append member \"{3}\"
                next\n
                    '''.format(lists[i].name[:-4],lists[j].name[:-4],lists[k].name[:-4],response[0]['dn'])
